...
export class SmsSimService implements OnModuleInit {
    ...
    async onModuleInit() {
        ...
        if (!port) {
            this.logger.warn(
                'SIM_PORT not configured. SMS SIM Service disabled.'
            );
            return;
        }
        ...
        try {
            ...
            this.modem.on('open', () => {
                ...
                this.modem.initializeModem((err: any) => {
                    if (err) {
                        this.logger.error(
                            `❌ Modem initialization failed: ${err.message}`
                        );
                        this.isModemReady = false;
                    } else {
                        this.isModemReady = true;
                        this.logger.log(
                            '✅ Modem initialized and ready for SMS.'
                        );
                    }
                });
            });
            ...
            this.modem.sendSMS(
                ...
                (response: any) => {
                    if (response?.data?.response === 'Message Sent') {
                        ...
                        reject(
                            new Error(
                                response?.data?.response || 'Failed to send SMS'
                            )
                        );
                    }
                }
