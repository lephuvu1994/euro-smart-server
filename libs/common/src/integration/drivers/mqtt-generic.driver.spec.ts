...
describe('MqttGenericDriver', () => {
  ...
  describe('setValueBulk', () => {
    ...
    it('should combine multiple entities into a single payload', async () => {
    ...
    it('should return false and log error if bulk publish fails', async () => {
      ...
      const result = await driver.setValueBulk({ token: 't' }, [{ commandKey: 'k', state: 1 } as any]);
  ...
  describe('publishRaw', () => {
    it('should publish raw payload to the device set topic', async () => {
      const result = await driver.publishRaw({ token: 'raw-token' }, { raw: 'data' });
