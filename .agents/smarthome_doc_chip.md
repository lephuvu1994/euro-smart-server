...
- **State (Trạng thái chi tiết)**: Cung cấp chính xác trạng thái vật lý của công tắc rèm: `OPENED` (cửa đã mở hết cỡ), `OPENING` (cửa đang chạy lên), `CLOSED` (cửa đã đóng kín), `CLOSING` (cửa đang chạy xuống), và `STOPPED` (cửa được dừng ở lưng chừng).
