...
export class VietguysService {
    ...
    async sendOtp(phone: string, otp: string): Promise<void> {
        if (!this.isEnabled) {
            this.logger.warn(
                `[VietGuys] Disabled. Mock OTP to ${phone}: ${otp}`
            );
            return;
        }
    ...
    private isVoiceEnabledAndConfigured(): boolean {
        return (
            this.configService.get<boolean>('vietguys.voice.enabled') ===
                true && !!this.configService.get<string>('vietguys.voice.url')
        );
    }
    ...
    private async getAccessToken(): Promise<string> {
        ...
        this.cachedAccessTokenExpiredAtSec =
            typeof json.data.expired_at === 'number'
                ? json.data.expired_at
                : null;
    ...
    private async sendZaloZns(phone: string, otp: string): Promise<void> {
        ...
        const extraJson =
            this.configService.get<string>(
                'vietguys.zalo.templateDataExtraJson'
            ) || '';
    ...
    private async sendSms(phone: string, otp: string): Promise<void> {
        ...
        if (!res.ok) {
            throw new Error(
                `SMS failed ${res.status} ${res.statusText}: ${text}`
            );
        }
