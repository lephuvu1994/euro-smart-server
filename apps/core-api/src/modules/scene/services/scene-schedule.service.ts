      ...
      for (let index = 0; index < triggers.length; index++) {
        ...
        if (config.cron) {
          if (this.isCronMatchingThisMinute(config.cron, tz, now)) {
            await this.tryFireScene(scene.id, scene.name, index, 'cron');
          }
        } else if (config.hour !== undefined && config.minute !== undefined) {
          if (this.isAtTimeMatching(config.hour, config.minute, tz, now)) {
            await this.tryFireScene(scene.id, scene.name, index, 'at');
          }
        }
      }
    }
